"use client";
import { generateUniqueId } from "@/app/terminal/terminal.api";
...
const RoleSelection = ({ className = "" }: { className?: string }) => {
  ...
  return (
    <RadioGroup
      ...
      className={cn(
        "w-fit h-fit flex items-start justify-between gap-2",
        className,
      )}
    >
      <Role
          ...
          <li key={"frontend" + "-" + item.id}>
            <div
              className={cn(
                ...
                selected === "frontend" ? "opacity-100" : "opacity-25",
              )}
            >
              <Checkbox
