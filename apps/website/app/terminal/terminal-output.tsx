...
const TerminalOutput = () => {
  ...
  return (
    <div className="terninal-output">
      {state.history.map((item, index) => {
        return <HistoryBlock key={`${item.timestamp}-${index}`} block={item} />;
      })}
    </div>
  );
};
      ...
      <div className="w-full flex items-center justify-between h-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-secondary">
            ({endDate !== null ? `${endDate}ms` : ""})
          </span>
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            className="size-6 p-0 group-hover:flex hidden"
          >
            <EllipsisVerticalIcon size={16} />
          </Button>
        </div>
      </div>
      <span className="w-full text-sm text-foreground font-semibold">
        {block.command}
      </span>
        ...
        return (
          <li key={item.message + "-" + index} className="list-item ">
            <div className="h-full flex items-center">
              ...
              {item.type === "stderr" && (
                <span className="text-sm text-error">{item.message}</span>
              )}
            </div>
          </li>
        );
